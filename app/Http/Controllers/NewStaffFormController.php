<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\User;

class NewStaffFormController extends Controller
{
    public function create()
    {
        $user = auth()->user(); 
        \Log::info('User ID: ' . $user->id);
        \Log::info('Department ID: ' . $user->department_id);
    
        $departmentName = $user->department ? $user->department->DepartmentName : 'Unknown Department';

        // Add this line for debugging
        \Log::info('Department Name: ' . $departmentName);
        $headOfDepartment = $user->department && $user->department->head ? $user->department->head->name : 'Unknown Head';
        $approvers=User::where('role_id',2)
                    ->where('department_id',$user->department_id)
                    ->get();
    
        return view('Ticket.newstaffForm',compact('approvers','departmentName','headOfDepartment'));
    }
}
